  ...
  award(awardNumbers){
    ...
    for(let i=0;i<QIUNUMBER;i++)
    {
      ...
      if(i<QIUNUMBER/2)
      {
        ...
        {
          xiadan = xiadan+this.state.touzhu[7-nhs.length];
            let zhong =':'+(hs==0?'龙':'虎')+this.state.touzhu[7-nhs.length]+'不中';
            awardInfo.push('第'+this.state.serverResult.current.periodNumber+'期第'+(i+1)+'名'+zhong);        
      ...
      if(i<QIUNUMBER/2)
        item = (
            <View style={styles.inText} key={i}>
              ...
              <View style={styles.inText2}>
                <Text style={styles.Text}>龙虎</Text>
                <Text style={[styles.Text3,hs[0]==1?{color:'red'}:{}]}>{hs[0]==undefined?null:(hs[0]==1?'龙':'虎')}{hs[0]==undefined?null:'('+xiazhu[7-hs.length]+')'}</Text>
              </View>
            </View>  );
      else
      {
